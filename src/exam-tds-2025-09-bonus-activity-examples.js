...
export async function questions(user, elementMap) {
  const results = [
    ...
    // New contributions for Jan26 graded prep (5 questions)
    {
      ...(await import('./q-weekend-days.js').then((m) => m.default({ user, weight: 0.5 }))),
      help: [
        html`<p>Use date functions (<code>NETWORKDAYS.INTL</code>, <code>datetime</code>, etc.) to count weekend days.</p>`,
      ],
    },
    {
      ...(await import('./q-calc-variance.js').then((m) => m.default({ user, weight: 0.5 }))),
      help: [
        html`<p>Population variance: <code>VAR.P</code> (Excel/Sheets) or <code>statistics.pvariance</code> (Python).</p>`,
      ],
    },
    {
      ...(await import('./q-regex-email-validation.js').then((m) => m.default({ user, weight: 0.5 }))),
      help: [html`<p>Pragmatic regex: <code>/^[A-Za-z0-9._%+-]+@[A-Za-z0-9.-]+\.[A-Za-z]{2,}$/</code></p>`],
    },
    {
      ...(await import('./q-html-table-to-markdown.js').then((m) => m.default({ user, weight: 0.5 }))),
      help: [html`<p>Convert HTML tables to pipe-style Markdown for docs/READMEs.</p>`],
    },
    {
      ...(await import('./q-csv-to-json.js').then((m) => m.default({ user, weight: 0.5 }))),
      help: [html`<p>When converting CSV to JSON, keep numeric fields numeric.</p>`],
    },
